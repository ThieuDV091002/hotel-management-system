...
const InventoryDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInventory = async () => {
      if (!accessToken) {
        ...
        toast.error('Please log in to access inventory details.');
        navigate('/login');
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.error('Failed to fetch inventory:', response.status, response.statusText, errorText);
          setError(`Failed to fetch inventory: ${errorText || response.statusText}`);
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    if (!formData.inventoryName.trim()) {
      toast.error('Invalid input: Inventory name cannot be empty.');
      return;
    }

    if (isNaN(formData.inventoryPrice) || formData.inventoryPrice < 0) {
      toast.error('Invalid input: Valid price is required.');
      return;
    }

    if (isNaN(formData.inventoryQuantity) || formData.inventoryQuantity < 0) {
      toast.error('Invalid input: Valid quantity is required.');
      return;
    }

    if (!formData.supplierName.trim()) {
      toast.error('Invalid input: Supplier name cannot be empty.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.error('Failed to update inventory:', response.status, response.statusText, errorText);
        toast.error(`Failed to update inventory: ${errorText || response.statusText}`);
      }
    } catch (error) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (formData) {
      setFormData({
        ...formData,
        [name]: name === 'inventoryPrice' || name === 'inventoryQuantity' ? parseFloat(value) || '' : value,
      });
    }
  };
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                {formData.inventoryPrice.toFixed(2)} VND
              </p>
            </div>
            <div>
              <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">
        ...
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Update inventory details to keep the profile up-to-date.
            </p>
          </div>
              ...
              <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                ...
                <div>
                  ...
                  <Input
                    type="text"
                    name="inventoryPrice"
                    value={formData.inventoryPrice.toString()}
                    onChange={handleInputChange}
                  />
                </div>
                <div>
