...
const SupplierDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      if (!accessToken) {
        ...
        toast.error('Please log in to access supplier details.');
        navigate('/login');
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.error('Failed to fetch supplier:', response.status, response.statusText, errorText);
          setError(`Failed to fetch supplier: ${errorText || response.statusText}`);
        }
      } catch (error) {
  ...
  const handleSave = async () => {
    ...
    if (!accessToken) {
      console.error('No access token found');
      toast.error('Please log in to update supplier.');
      navigate('/login');
      return;
    }
    ...
    if (!formData.supplierAddress.trim()) {
      toast.error('Supplier address cannot be empty.');
      return;
    }

    if (!formData.supplierPhone.trim()) {
      toast.error('Supplier phone cannot be empty.');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorText = await response.text();
        console.error('Failed to update supplier:', response.status, response.statusText, errorText);
        toast.error(`Failed to update supplier: ${errorText || response.statusText}`);
      }
    } catch (error) {
    ...
    <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
      ...
      <div className="mt-8">
        ...
        {formData.inventoryItems && formData.inventoryItems.length > 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {formData.inventoryItems.map((item) => (
                    <tr key={item.id}>
                      <td className="px-4 py-3 text-gray-500 text-start text-sm dark:text-gray-400">{item.id}</td>
                      <td className="px-5 py-4 sm:px-6 text-start text-sm text-gray-800 dark:text-white/90">{item.name}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">No inventory items associated with this supplier.</p>
        )}
      </div>
        ...
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Update supplier details to keep the profile up-to-date.
            </p>
          </div>
